...
def test_Qualean_comparison():
    ...
    assert min(r1,r2) < max(r1,r2), "comparison is not working fine"  # using min, max function to be sure which one is smaller
...
def test_Qualean_equality():
    ...
    assert r2 == r1, "comparison is not working fine"  # using min, max function to be sure which one is smaller

def test_Qualean_invertsign():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    r2 = (-1)*r1.number  # storing the negative of object
    assert r1.__invertsign__() == r2, "comparison is not working fine"
...
def test_100_times_sum():
    ...
    for i in range(100):
        r2 = r2 + r1.number
    assert 100*r1.number == r2 , "comparison is not working fine "
...
def test_1_Million_Qualean_mean_closeto_zero():
    ...
    for i in range(1000000):
        ...
        r2 = r2 + r1.number
    r2 = r2/1000000
    assert isclose(decimal.Decimal(0), r2, rel_tol=0.01,abs_tol=0.01), "comparison is not working fine"

def test_Qualean_BOOL():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    assert r1.__bool__() == bool(r1), "comparison is not working fine"

def test_Qualean_AND():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    r2 = session4.Qualean(number_2)  # this will be a random decimal number
    assert (r1.__and__(r2)) == bool(r1 and r2), "comparison is not working fine"

def test_Qualean_OR():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    r2 = session4.Qualean(number_2)  # this will be a random decimal number
    assert (r1.__or__(r2)) == bool(r1 or r2), "comparison is not working fine"

def test_Qualean_AND_short_Circuit():
    r1 = session4.Qualean(0)  # this will be a 0 decimal number
    # r2 is not defined
    assert bool(r1 and r2) == False, "comparison is not working fine"

def test_Qualean_OR_short_Circuit():
    r1 = session4.Qualean(1)  # this will be a random decimal number
    # r2 is not defined
    assert bool(r1 or r2) == True, "comparison is not working fine"

def test_Qualean_ADD():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    r2 = session4.Qualean(number_2)  # this will be a random decimal number
    assert (r1.__add__(r2)) == (r1.number + r2.number), "comparison is not working fine"

def test_Qualean_MUL():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    r2 = session4.Qualean(number_2)  # this will be a random decimal number
    assert (r1.__mul__(r2)) == (r1.number * r2.number), "comparison is not working fine"

def test_Qualean_SQRT():
    r1 = session4.Qualean(number_1)  # this will be a random decimal number
    r1.number = abs(r1.number)
    assert r1.__sqrt__() == r1.number.sqrt(), "comparison is not working fine"

def test_Qualean_SQRT_Negative():
    r1 = session4.Qualean(1)  # this will be a random decimal number
    r1.number = abs(r1.number)*(-1)
    assert type(r1.__sqrt__()) == complex, "comparison is not working fine"
